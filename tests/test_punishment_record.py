...
class PunishmentRecordTests(unittest.IsolatedAsyncioTestCase):
    """PunishmentRecordRepository 与 PunishmentHistoryModal 的单元测试。"""
    ...
    async def test_summary_is_scoped_counted_and_limited(self) -> None:
        ...
        async with AsyncSession(self.engine) as session:
            ...
            # 查询目标帖子的汇总数据
            total, records = await repository.get_summary(thread_id=10, target_user_id=20)
    ...
    async def test_empty_modal_has_summary_only(self) -> None:
        ...
        summary_display = cast(discord.ui.TextDisplay, modal.children[0])
        self.assertIn("暂无处罚记录", summary_display.content)
