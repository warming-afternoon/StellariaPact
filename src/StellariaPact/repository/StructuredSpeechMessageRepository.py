...
class StructuredSpeechMessageRepository:
    ...
    async def get_original_user_id(self, message_id: int) -> int | None:
        """按 Discord 消息 ID 查询结构化发言的原提交者 ID。"""
        result = await self.session.exec(
            select(StructuredSpeechMessage.user_id).where(
                StructuredSpeechMessage.message_id == message_id
            )
        )
