...
class VotingService:
    ...
    def get_vote_details_dto(
        ...
        if vote_session_model.total_choices > 1 and vote_options:  # type: ignore
            ...
            # 单选项兼容模式
            approve = sum(1 for v in all_votes if v.choice == 1)
            reject = sum(1 for v in all_votes if v.choice == 0)
            option_results.append(
                OptionResult(
                    choice_index=1,
                    choice_text="提案",
