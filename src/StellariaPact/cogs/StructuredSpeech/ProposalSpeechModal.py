...
class ProposalSpeechModal(discord.ui.Modal, title="提案发言"):
    ...
    def __init__(
        ...
        *,
        ...
        if (reference_message_url is None) != (reference_user_id is None):
            raise ValueError("源消息链接和目标用户 ID 必须同时提供。")
        self.cog = cog
        self.thread_id = thread_id
        self.user_id = user_id
        self.reference_message_url = reference_message_url
        self.reference_user_id = reference_user_id

        # 说明手动提及格式，并提供不需要复制用户 ID 的右键入口。
        self.add_item(
            discord.ui.TextDisplay(
                "提及用户时请填写 `<@用户ID>`。如果不知道用户 ID，可右键目标消息，"
                "选择“应用”→“提案发言(回复)”，Bot 会自动添加源消息链接和用户提及。"
            )
        )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        await self.cog.submit_proposal_speech(
            ...
            reference_message_url=self.reference_message_url,
            reference_user_id=self.reference_user_id,
        )
