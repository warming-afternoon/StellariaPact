...
class PunishmentCog(commands.Cog):
    ...
    def __init__(self, bot: StellariaPactBot):
        ...
        self.manage_punishment_ctx = app_commands.ContextMenu(
            name="管理处罚", callback=self.manage_punishment_user, type=discord.AppCommandType.user
        )
    ...
    async def cog_unload(self) -> None:
        self.bot.tree.remove_command(self.kick_proposal_ctx.name, type=self.kick_proposal_ctx.type)
        self.bot.tree.remove_command(self.manage_punishment_ctx.name, type=self.manage_punishment_ctx.type)

    @RoleGuard.requireRoles("councilModerator")
    async def kick_proposal_message(self, interaction: discord.Interaction, message: discord.Message):
    ...
    @RoleGuard.requireRoles("councilModerator")
    async def manage_punishment_user(self, interaction: discord.Interaction, member: discord.Member):
        ...
        async with UnitOfWork(self.bot.db_handler) as uow:
            activity = await uow.user_activity.get_user_activity(member.id, interaction.channel.id) # type: ignore
    ...
    async def _validate_context(self, interaction: discord.Interaction, target_user: discord.User | discord.Member) -> bool:
        """验证命令是否在正确的上下文中使用"""
