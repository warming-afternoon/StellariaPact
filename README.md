...
graph TD
    ...
    Discuss -->|"/进入执行<br/>(讨论≥24h)"| Execute
    
    %% 正常结束流程
    Discuss -->|"/提案完成<br/>"| Ended
    Execute -->|"/提案完成<br/>"| Ended

    %% 废弃流程
    Discuss -->|"/废弃<br/>"| Abandoned
    Objection -->|"/废弃<br/>"| Abandoned
    Execute -->|"/废弃<br/>"| Abandoned

    %% 回滚流程 (虚线)
    Objection -.->|"/重新讨论<br/>(异议>12h且无通过异议)"| Discuss
    Execute -.->|"/重新讨论<br/>"| Discuss
    Ended -.->|"/重新讨论<br/>"| Discuss
    Abandoned -.->|"/重新讨论<br/>"| Discuss
...
- 说明: 已有投票、支持票和附议不会被删除，处罚期间仍可主动撤回；重复处罚会从执行时重新计算期限并保留历史记录。
...
- 权限: `管理组` / `议事督导` / `执行监理`
- 说明: 查询包含生效中、已到期、已解除和被覆盖的记录，结果仅操作人可见。
